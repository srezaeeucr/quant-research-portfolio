#!/usr/bin/env python3
"""
All-Day Gap Fill — fills the missing all-day coverage for symbols that
were never tested with extended entry windows.

Usage:
    python backtest/allday_gapfill.py --symbols META AMZN JPM QQQ IWM TSLA
    python backtest/allday_gapfill.py --symbols COIN MSTR XLK XLF XLI
    python backtest/allday_gapfill.py --symbols XLV
    python backtest/allday_gapfill.py --dry-run --symbols META AMZN
"""
import os
...
ALL_MISSING = ["META", "AMZN", "JPM", "QQQ", "IWM", "TSLA",
               "COIN", "MSTR", "XLK", "XLF", "XLI", "XLV"]
...
def _generate_combos(grid, symbols):
    ...
    for strategy, symbol, period in itertools.product(
        grid["strategies"], symbols, grid["periods"]
    ):
...
def main():
    ...
    parser.add_argument("--symbols", nargs="+", default=ALL_MISSING)
    parser.add_argument("--workers", type=int, default=None)
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    combos = _generate_combos(GRID_TEMPLATE, args.symbols)
    total = len(combos)

    print(f"\n  All-Day Gap Fill")
    print(f"  Strategies : {GRID_TEMPLATE['strategies']}")
    print(f"  Symbols    : {args.symbols}")
    print(f"  Windows    : {GRID_TEMPLATE['entry_window_minutes']} min (→ 12:45, 15:45 ET)")
    print(f"  Periods    : {len(GRID_TEMPLATE['periods'])} (incl. holdout_2022)")
